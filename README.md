...
| module-dependency-order   | Module dependencies should be sorted in a logical manner. This rule provides two ways to sort modules, combined or grouped. In combined mode, all modules should be sorted alphabetically. In grouped mode the modules should be grouped in the order: standard modules - third party modules - custom modules. A prefix can be specified to determine which prefix the custom modules have. ('module-dependency-order', [2, {mode: "grouped", prefix: "app"}]) |
